...
(() => {
  ...
  const TR_RU = {
    ...
    "בילויים": "Развлечения", "חינוך": "Образование", "חיות מחמד": "Питомцы", "אחר": "Другое",
  ...
  const TR_EN = {
    ...
    "בילויים": "Entertainment", "חינוך": "Education", "חיות מחמד": "Pets", "אחר": "Other",
  ...
  const PRESETS = {
    generic: {
      expense: [
        ...
        { id: "pets", name: "חיות מחמד", icon: "🐾", color: "#a16207" },
  ...
  /* ---------- סניטציה: כל מה שנכנס מבחוץ (קובץ ייבוא) או מהאחסון עובר רשימה לבנה ----------
     טקסט חופשי (תיאור/שם) נשמר כמו שהוא ומקודד ב-render; שדות שנכנסים ל-attributes/class
     (id/type/date/category) חייבים להתאים לתבנית, אחרת הרשומה נזרקת. */
  const SAFE_ID = /^[A-Za-z0-9_:-]{1,80}$/, SAFE_DATE = /^\d{4}-\d{2}-\d{2}$/, SAFE_YM = /^\d{4}-\d{2}$/, SAFE_HEX = /^#[0-9a-fA-F]{6}$/;
  const cleanStr = (s, max) => (s == null ? "" : String(s)).slice(0, max);
  const cleanText = (s, max) => cleanStr(s, max).replace(/[<>&"']/g, "");      // לשדות שמוצגים בלי escapeHtml
  const cleanNum = (n) => (Number.isFinite(+n) ? +n : NaN);
  const cleanType = (t) => (t === "income" ? "income" : "expense");
  const okId = (s) => SAFE_ID.test(String(s));
  function sanitizeTx(t) {
    if (!t || typeof t !== "object" || !okId(t.id) || !SAFE_DATE.test(String(t.date)) || !okId(t.category)) return null;
    const amount = cleanNum(t.amount); if (!(amount >= 0)) return null;
    const out = { id: String(t.id), type: cleanType(t.type), amount: round2(amount), category: String(t.category), description: cleanStr(t.description, 200), date: String(t.date) };
    if (t.recurringId && okId(t.recurringId)) out.recurringId = String(t.recurringId);
    if (t.monthly) out.monthly = true;
    return out;
  }
  function sanitizeGoal(g) {
    if (!g || typeof g !== "object" || !okId(g.id)) return null;
    const target = cleanNum(g.target); if (!(target > 0)) return null;
    const saved = cleanNum(g.saved);
    return { id: String(g.id), name: cleanStr(g.name, 60), target: round2(target), saved: saved > 0 ? round2(saved) : 0, emoji: (/^[^\w\s]{1,4}$/u.test(String(g.emoji || "")) ? String(g.emoji) : "🎯") };  // אימוג'י בלבד, אחרת ברירת מחדל
  }
  function sanitizeRec(r) {
    if (!r || typeof r !== "object" || !okId(r.id) || !okId(r.category)) return null;
    const amount = cleanNum(r.amount); if (!(amount > 0)) return null;
    const day = Math.round(cleanNum(r.day)); if (!(day >= 1 && day <= 31)) return null;
    const out = { id: String(r.id), type: cleanType(r.type), amount: round2(amount), category: String(r.category), description: cleanStr(r.description, 200), day };
    if (SAFE_YM.test(String(r.createdMonth))) out.createdMonth = String(r.createdMonth);
    return out;
  }
  function sanitizeBudgets(b) {
    const out = {};
    if (b && typeof b === "object") Object.keys(b).forEach((k) => { const v = cleanNum(b[k]); if (SAFE_YM.test(k) && v > 0) out[k] = Math.round(v); });
    return out;
  }
  function sanitizeApplied(a) {
    const out = {};
    if (a && typeof a === "object") Object.keys(a).forEach((k) => { if (/^[A-Za-z0-9_-]{1,80}:\d{4}-\d{2}$/.test(k) && a[k]) out[k] = true; });
    return out;
  }
  const sanitizeList = (arr, fn) => (Array.isArray(arr) ? arr.map(fn).filter(Boolean) : []);
  // קטגוריה מותאמת: שם/אייקון מוצגים ב-innerHTML ב-15 מקומות — מנקים במקור במקום בכל אחד מהם
  function sanitizeCat(c) {
    if (!c || typeof c !== "object" || !okId(c.id)) return null;
    const name = cleanText(c.name, 40).trim(); if (!name) return null;
    return { id: String(c.id), name, icon: cleanText(c.icon, 8) || "🏷️", color: SAFE_HEX.test(String(c.color)) ? String(c.color) : "#64748b" };
  }
  function sanitizeCats(cc) {
    return { expense: sanitizeList(cc && cc.expense, sanitizeCat), income: sanitizeList(cc && cc.income, sanitizeCat) };
  }
  customCats = sanitizeCats(customCats); // גם מה שכבר באחסון — הגנה לעומק
  ...
  el.importFile.addEventListener("change", (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!raw || !Array.isArray(raw.transactions)) throw new Error("bad");
        // הקובץ הוא קלט לא אמין: כל רשומה עוברת רשימה לבנה, מה שלא עובר — נזרק בשקט
        const data = {
          transactions: sanitizeList(raw.transactions, sanitizeTx),
          budgets: sanitizeBudgets(raw.budgets),
          goals: sanitizeList(raw.goals, sanitizeGoal),
          recurring: sanitizeList(raw.recurring, sanitizeRec),
          recApplied: sanitizeApplied(raw.recApplied),
        };
        ...
        if (merge) {
          ...
          const gids = new Set(goals.map((g) => g.id)); data.goals.forEach((g) => { if (!gids.has(g.id)) goals.push(g); });
          const rids = new Set(recurring.map((r) => r.id)); data.recurring.forEach((r) => { if (!rids.has(r.id)) recurring.push(r); });
  ...
  el.clearAll.addEventListener("click", () => {
    const hasCatBudgets = Object.keys(catBudgets).length > 0;
    if (!transactions.length && !Object.keys(budgets).length && !goals.length && !recurring.length && !hasCatBudgets) { toast("אין נתונים למחיקה"); return; }
    ...
    // הקטגוריות שהמשתמש הוסיף נשארות — הן מבנה, לא נתונים. הסכומים המתוכננים שלהן מתאפסים.
    transactions = []; budgets = {}; goals = []; recurring = []; recApplied = {};
    Object.keys(catBudgets).forEach((k) => delete catBudgets[k]);
  ...
  el.catAdd.addEventListener("click", () => {
    ...
    const cat = sanitizeCat({ id: "custom_" + uid(), name, icon: newCatEmoji, color: el.newCatColor.value });
    if (!cat) { toast("נא להזין שם לקטגוריה"); return; }
