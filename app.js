...
(() => {
  ...
  function buildDemoData() {
    ...
    months.forEach((ym) => {
      ...
      // הכנסות
      tx.push(mk(ym, "income", inc[0].id, kid ? money(60, 100) : money(6500, 8200), "", 2));
      if (inc[1]) tx.push(mk(ym, "income", inc[1].id, kid ? money(20, 70) : money(300, 1400), "", Math.min(12, maxDay)));
      // הוצאות — מכסה את רוב הקטגוריות כדי שגרף העוגה יהיה עשיר
      const n = kid ? Math.min(exp.length, 6) : Math.min(exp.length, 8);
      for (let i = 0; i < n; i++) {
        const c = exp[i % exp.length];
        const amt = kid ? money(5, 45) : (i === 0 ? money(700, 1500) : money(40, 550));
        tx.push(mk(ym, "expense", c.id, amt, descs[i % descs.length], 2 + Math.floor(Math.random() * Math.max(1, maxDay - 2))));
      }
    });
    const budgets = {};
    months.slice(-3).forEach((ym) => budgets[ym] = kid ? 300 : 8000); // תקציב ל-3 החודשים האחרונים
    const catBudgets = {};
